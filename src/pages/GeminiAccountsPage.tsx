...
export function GeminiAccountsPage() {
  ...
  const page = useProviderAccountsPage<GeminiAccount>({
    ...
    oauthService: {
      ...
      completeLogin: (loginId: string) =>
        geminiService.completeGeminiOAuthLogin(loginId),
      cancelLogin: (loginId?: string) =>
        geminiService.cancelGeminiOAuthLogin(loginId),
    ...
    onInjectSuccess: async ({ accountId, account, displayEmail }) => {
      try {
        const launchInfo =
          await geminiInstanceService.getGeminiInstanceLaunchCommand(
            "__default__",
          );
        setLaunchModal({
        ...
        console.error(
          "[GeminiAccountsPage] load default launch command failed:",
          error,
        );
      }
    },
  ...
  const handleExecuteInTerminal = useCallback(async () => {
    ...
    setLaunchModal((prev) =>
      prev
        ? { ...prev, executing: true, executeMessage: null, executeError: null }
        : prev,
    );
    try {
      const result =
  ...
  const tierSummary = useMemo(() => {
    ...
    const extraKeys = Array.from(dynamicCounts.keys())
      .filter(
        (tier) =>
          !(CURSOR_KNOWN_PLAN_FILTERS as readonly string[]).includes(tier),
      )
  ...
  const tierFilterOptions = useMemo<MultiSelectFilterOption[]>(() => {
    const options: MultiSelectFilterOption[] = [
      {
        value: "FREE",
        label: resolveFilterLabel("FREE", tierSummary.knownCounts.FREE),
      },
      {
        value: "PRO",
        label: resolveFilterLabel("PRO", tierSummary.knownCounts.PRO),
      },
      {
        value: "PRO_PLUS",
        label: resolveFilterLabel("PRO_PLUS", tierSummary.knownCounts.PRO_PLUS),
      },
      {
        value: "ENTERPRISE",
        label: resolveFilterLabel(
          "ENTERPRISE",
          tierSummary.knownCounts.ENTERPRISE,
        ),
      },
      {
        value: "FREE_TRIAL",
        label: resolveFilterLabel(
          "FREE_TRIAL",
          tierSummary.knownCounts.FREE_TRIAL,
        ),
      },
      {
        value: "ULTRA",
        label: resolveFilterLabel("ULTRA", tierSummary.knownCounts.ULTRA),
      },
    ];
    tierSummary.extraKeys.forEach((planKey) => {
      options.push({
        ...
        label: resolveFilterLabel(
          planKey,
          tierSummary.dynamicCounts.get(planKey) ?? 0,
        ),
      });
    });
    ...
    tierSummary.knownCounts.ENTERPRISE,
    tierSummary.knownCounts.FREE,
    tierSummary.knownCounts.FREE_TRIAL,
    tierSummary.knownCounts.PRO,
    tierSummary.knownCounts.PRO_PLUS,
    tierSummary.knownCounts.ULTRA,
  ...
  const filteredAccounts = useMemo(() => {
    ...
    if (filterTypes.length > 0) {
      ...
      if (selectedTypes.size > 0) {
        result = result.filter((account) =>
          selectedTypes.has(resolvePlanKey(account)),
        );
      }
    }
            ...
            <div className="account-cell">
              <div className="account-main-line">
                <span
                  ...
                  title={maskAccountText(emailText)}
                >
                  {maskAccountText(emailText)}
                </span>
                {planLabel && planLabel !== "UNKNOWN" && (
        ...
        {!isFlowNoticeCollapsed && (
          <div className="ghcp-flow-notice-body">
            <div className="ghcp-flow-notice-desc">
              {t(
                "gemini.flowNotice.desc",
                "Manage Gemini accounts by importing from local Gemini installation or pasting JWT tokens. Data is processed locally only.",
              )}
            </div>
            <ul className="ghcp-flow-notice-list">
              <li>
                {t(
                  "gemini.flowNotice.reason",
                  "Permission scope: read Gemini local auth storage for account import and token injection.",
                )}
              </li>
              <li>
                {t(
                  "gemini.flowNotice.storage",
                  "Data scope: only Gemini auth-session related entries are read/updated; no key/token is uploaded.",
                )}
              </li>
            </ul>
          </div>
                            ...
                            <div className="oauth-link">
                              ...
                              <div className="oauth-url-box oauth-manual-input">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setOauthManualCallbackInput(e.target.value)
                                  }
                                  placeholder={t(
